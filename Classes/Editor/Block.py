...
class EmptyBlock(pygame.sprite.Sprite):
    ...
    def Update(self):
        if (self.OnButton() and bool(pygame.mouse.get_pressed()[0] == 1)):
            if (Tools.selectedTool == "Paint"):
                self.image = pygame.image.load(LEVEL_EDITOR_SPRITES[DynamicSettings.WORLD_TYPE][Tools.Paint.imageName])
    ...
    def OnButton(self):
        ...
        inDraw = 0 <= pos[0] and DRAW_WIDTH > pos[0] and 0 <= pos[1] and DRAW_HEIGHT > pos[1]
        return self.rectTwo.x <= pos[0] and self.rectTwo.x + self.rectTwo.width > pos[0] and self.rectTwo.y <= pos[1] and self.rectTwo.y + self.rectTwo.height > pos[1] and inDraw
...
class TileBlock(pygame.sprite.Sprite):
    def __init__(self, x, y, name="EmptyBlock"):
        ...
        if (name in EDITOR_SPRITES.keys()):
            ...
            self.image = pygame.image.load(LEVEL_EDITOR_SPRITES[DynamicSettings.WORLD_TYPE][name])
